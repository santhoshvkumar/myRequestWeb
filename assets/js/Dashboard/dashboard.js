        ...
        function getTenantUtilityStatus(adminUserID, Getcountry){
            ...
            $.get(domainAddress+getUtilityStatusUrl,{},function(resultTenantUtility){
                ...
                if(resultTenantUtility.record_count==0){
                    ...
                    for(tenant in resultTenantUtility.records){
                        ...
                        $(".tenantUtility").append("<tr id='getTenantRowID-" + resultTenantUtility.records[tenant].TenantID + "'><td id='propName-" + resultTenantUtility.records[tenant].TenantID + "' style='vertical-align: middle;'>" + resultTenantUtility.records[tenant].TenantName + "</td> <td id='propAddress-" + resultTenantUtility.records[tenant].TenantID + "' style='vertical-align: middle;'>"+resultTenantUtility.records[tenant].PropAddress+"</td><td id='tenancyEndDate-" + resultTenantUtility.records[tenant].TenantID + "' style='vertical-align: middle;'>"+moment(resultTenantUtility.records[tenant].TenancyEnd).format('Do MMM YYYY') +"</td> <td id='getTenantRenevalRowID-" + resultTenantUtility.records[tenant].TenantID + "'>  <i class='fa fa fa-refresh fa-2x reneval' style='color:green;cursor:pointer;' id='getTenantRenevalID-"+resultTenantUtility.records[tenant].TenantID+"'></i> </td><td>  <i class='fa fa fa-sign-out fa-2x moveOut' style='color:red;cursor:pointer;' id='moveOut-"+resultTenantUtility.records[tenant].TenantID+"'></i> <input type='hidden' id='hiddenPropertyID-"+resultTenantUtility.records[tenant].TenantID+"' value='"+resultTenantUtility.records[tenant].PropertyRegister+"' /> <input type='hidden' id='hiddenTenantName-"+resultTenantUtility.records[tenant].TenantID+"' value='"+resultTenantUtility.records[tenant].TenantName+"' /> <input type='hidden' id='hiddenTenantEmailID-"+resultTenantUtility.records[tenant].TenantID+"' value='"+resultTenantUtility.records[tenant].TenantEmailID+"' /> <input type='hidden' id='hiddenTenantPhoneNumber-"+resultTenantUtility.records[tenant].TenantID+"' value='"+resultTenantUtility.records[tenant].TenantPhoneNumber+"' /> <input type='hidden' id='hiddenTenantStartDate-"+resultTenantUtility.records[tenant].TenantID+"' value='"+resultTenantUtility.records[tenant].TenancyStart+"' /> <input type='hidden' id='hiddenTenantEndDate-"+resultTenantUtility.records[tenant].TenantID+"' value='"+resultTenantUtility.records[tenant].TenancyEnd+"' /> <span id='utilityInfo-"+resultTenantUtility.records[tenant].TenantID+"'></span> </td> </tr> ");
                    ...
                    $(".reneval").on('click',function(){
                        var tenantID = this.id.replace('getTenantRenevalID-','');
                        var tenantName = $("#propName-"+tenantID).text();
                        var tenantPropertyAddress = $("#propAddress-"+tenantID).text();
                        var tenantNewStartDate = $("#hiddenTenantStartDate-"+tenantID).val();
            ...
            $.get(domainAddress+getUtilityStatusUrl,{},function(resultContractorUtility){
                ...
                if(resultContractorUtility.record_count==0){
                    $(".contractorUtility").append("<tr><td id='ContractorName--0'>No records found</td> <td id='ContractorSpeciality-0'></td> <td id='ContractorEndDate-0'></td></tr>");
                }
                else{
     ...
     $.get(domainAddress + getMessageUrl, {}, function(result) {
        ...
        if (result.record_count == 0) {
            ...
            for (workLogNotes in result.records) {
                ...
                $(".messageNotesContent").append("<li> <div class='uk-grid' data-uk-grid-margin> <div class='uk-width-2-10'> <img src='"+getImage+"' class='imgUserCircle'/> </div>  <div class='uk-width-7-10'>  <div class='uk-grid' data-uk-grid-margin> <div class='uk-width-6-10'> <h3><b>"+result.records[workLogNotes].WorkAssignedBy+"</b></h3> </div> <div class='uk-width-4-10'> <span class='dashWorkLogDate'>"+moment(result.records[workLogNotes].WorkCreatedDate).format('Do MMM YYYY,  h:mm a')+"</span> </div> </div> <p class='dashWorkLogContent'>"+result.records[workLogNotes].Content+"</p> <p class='dashWorkLogCaseNo'> <a class='getRequestID' id='getRequestID-"+result.records[workLogNotes].ProblemID+"' style='cursor:poiter;'>( Request # "+result.records[workLogNotes].RequestID+" )</a> </p> </div> </div> </li> <hr/>");
            }   

            $(".getRequestID").on('click', function() {
                var getRequestID = this.id.replace('getRequestID-', '');
                console.log(getRequestID);
                localStorage.setItem("ParticularProblemID", getRequestID);
                window.location.href="https://agent.myrequest.co.uk/particularProblem.php";
            });
